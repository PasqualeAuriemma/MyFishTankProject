    ...
    <script type="text/javascript">
      ...
      function drawPieAreaColumnCharts() {
        ...
        var options_valEC = {  
          height: '1400px',
          width: '100px',
          title: 'EC daily average is: $formatted_temperature and Conductivity trend',
          hAxis: {textPosition: 'none'},
          explorer: { maxZoomIn: .5 , maxZoomOut: 8 },
          backgroundColor: '#f1f8e9',
          vAxis: {'title': 'Conductivity', 'minValue': 680, 'maxValue': 850},
          chartArea:{
            top: 36,
            left: 100,
            right: 18,
        ...
        buttonEC7D.onclick = function () {
          ...
          data_valEC = google.visualization.arrayToDataTable([
            ...
            ['Date', 'EC'],
            <?php         
            $query51 = "SELECT AVG(ec) as media, FROM_UNIXTIME(data_send, '%Y-%m-%d') as data FROM my_myfishtank.ec_tab WHERE FROM_UNIXTIME(data_send, '%Y-%m-%d') >= '$dataNow' GROUP BY data ORDER BY data";
            $result51 = $con->query($query51);
        ...
        buttonECALL.onclick = function () {
          ...
          data_valEC = google.visualization.arrayToDataTable([
           ...
           data_valEC5 = google.visualization.arrayToDataTable([
            ['Date', 'EC'],
            <?php         
            $query51 = "SELECT AVG(ec) as media, FROM_UNIXTIME(data_send, '%Y-%m-%d') as data FROM my_myfishtank.ec_tab GROUP BY data ORDER BY data";
            $result51 = $con->query($query51);
            if ($result51->num_rows > 0) {
            // output data of each row
              while($row51 = $result51->fetch_assoc()) {
