  ...
  <body class="homepage is-preload">
    <div id="page-wrapper">
      ...
      <section id="tdsec" class="wrapper style5">
        <div class="title">Conductivity and Total Dissolved Solids</div>
        <div class="container">
            ...
            <div class="col-6 col-12-medium">
              <section class="highlight">
                <p class="style2">
                  ...
                  <a href="#!" data-id="" data-bs-toggle="modal" data-bs-target="#addConductivityModal"   class="btn btn-success btn-sm" >Add EC Value</a>
                </p>
                  ...
                  <?php
                    ...
                    if ($result_list->num_rows > 0) {
                      while($row4 = $result_list->fetch_assoc()) {
                        echo "<li class='style1'> At ".gmdate("H:i:s\ ", $row4['date'])."  TDS = ".round($row4['tds'])." ppm - EC = ".number_format($row4['ec'], 1)." µS/cm</li>";
                      }
                    }
                    echo '</ol>';
                  ?>
          ...
          <div id="areachartECD" style="margin: 0px 2px;"></div>
...
<!--  <section id="main" class="wrapper style4">
    ...
    <script type="text/javascript">
			$(document).on('submit','#addConductivity',function(e){
      			e.preventDefault();
                ...
                if (key == "Pia12"){
                       $.ajax({
                         ...
                         success:function(data){
                           ...
                           if(status=='true'){
                            $('#addConductivityModal').modal('hide');
                            
                            var frm = document.getElementsByName('addConductivity')[0];
    ...
    <script type="text/javascript">
      ...
      function setDataForAreaChart(){
        ...
        $year = date('Y');
        $data7DayFromNow = date('Y-m-d', strtotime("-7 day"));
        $query4 = "SELECT AVG(ec) as media FROM my_myfishtank.ec_tab WHERE FROM_UNIXTIME(data_send, '%Y-%m-%d') = '$todayData'";
        $query5 = "SELECT AVG(ec) as media, FROM_UNIXTIME(data_send, '%Y-%m-%d') as data FROM my_myfishtank.ec_tab WHERE FROM_UNIXTIME(data_send, '%Y-%m-%d') >= '$dataNow'  GROUP BY data ORDER BY data";
        ...
        $quer2EC = "SELECT e.data_arrive as date, e.ec as E, t.tds as T FROM my_myfishtank.tds_tab t JOIN my_myfishtank.ec_tab e ON t.id = e.id WHERE FROM_UNIXTIME(e.data_send, '%Y-%m-%d') >= '$dataNow' ORDER BY e.data_send";
        $queryE7D = "SELECT e.data_arrive as date, e.ec as E, t.tds as T FROM my_myfishtank.tds_tab t JOIN my_myfishtank.ec_tab e ON t.id = e.id WHERE FROM_UNIXTIME(e.data_send, '%Y-%m-%d') >= '$data7DayFromNow' ORDER BY e.data_send";          
        $queryM7D = "SELECT AVG(ec) as media, FROM_UNIXTIME(data_send, '%Y-%m-%d') as data FROM my_myfishtank.ec_tab WHERE FROM_UNIXTIME(data_send, '%Y-%m-%d') >= '$data7DayFromNow' GROUP BY data ORDER BY data";
        $data2MonthsFromNow = date('Y-m-d', strtotime("-2 month"));        
        $data1MonthsFromNow = date('Y-m-d', strtotime("-1 month"));
        $queryE2M = "SELECT e.data_arrive as date, e.ec as E, t.tds as T FROM my_myfishtank.tds_tab t JOIN my_myfishtank.ec_tab e ON t.id = e.id WHERE FROM_UNIXTIME(e.data_send, '%Y-%m-%d') >= '$data2MonthsFromNow' ORDER BY e.data_send";          
        $queryM2M = "SELECT AVG(ec) as media, FROM_UNIXTIME(data_send, '%Y-%m-%d') as data FROM my_myfishtank.ec_tab WHERE FROM_UNIXTIME(data_send, '%Y-%m-%d') >= '$data2MonthsFromNow' GROUP BY data ORDER BY data";
        $queryE1M = "SELECT e.data_arrive as date, e.ec as E, t.tds as T FROM my_myfishtank.tds_tab t JOIN my_myfishtank.ec_tab e ON t.id = e.id WHERE FROM_UNIXTIME(e.data_send, '%Y-%m-%d') >= '$data1MonthsFromNow' ORDER BY e.data_send";          
        $queryM1M = "SELECT AVG(ec) as media, FROM_UNIXTIME(data_send, '%Y-%m-%d') as data FROM my_myfishtank.ec_tab WHERE FROM_UNIXTIME(data_send, '%Y-%m-%d') >= '$data1MonthsFromNow' GROUP BY data ORDER BY data";
        $queryEALL = "SELECT e.data_arrive as date, e.ec as E, t.tds as T FROM my_myfishtank.tds_tab t JOIN my_myfishtank.ec_tab e ON t.id = e.id ORDER BY e.data_send";          
        $queryMALL = "SELECT AVG(ec) as media, FROM_UNIXTIME(data_send, '%Y-%m-%d') as data FROM my_myfishtank.ec_tab GROUP BY data ORDER BY data";
      ...
      function drawPieAreaColumnCharts() {
        ...
        data_valEC5 = google.visualization.arrayToDataTable([
          ['Date', 'EC'],
          <?php // PHP Google Charts
          include("connection.php");
        ...
        var options_valEC5 = {  
          height: '1400px',
          width: '100px',
          title: 'Daily Average Conductivity trend',
          hAxis: {textPosition: 'none'},
          explorer: { maxZoomIn: .5 , maxZoomOut: 8 },
          backgroundColor: '#f1f8e9',
          vAxis: {'title': 'Daily Conductivity', 'minValue': 680, 'maxValue': 850},
          chartArea:{
            top: 36,
            left: 100,
            right: 18,
        ...
        data_valEC = google.visualization.arrayToDataTable([
          ...
          if ($result1E->num_rows > 0) {
            ...
            while($row1E = $result1E->fetch_assoc()) {
              echo "['".$row1E['date']."',".$row1E['E'].",".$row1E['T']."],";
            }
          }
          ?>
        ]);
        ...
        var options_valEC = {  
          height: '1400px',
          width: '100px',
          title: 'Conductivity trend and Total Dissolved Solids trend - EC daily average is: $formatted_temperature',
          hAxis: {textPosition: 'none'},
          explorer: { maxZoomIn: .5 , maxZoomOut: 8 },
          backgroundColor: '#f1f8e9',
          vAxis: {'title': 'Conductivity & TDS', 'minValue': 380, 'maxValue': 850},
          chartArea:{
            top: 36,
            left: 100,
            right: 18,
        ...
        var chart_valEC5 = new google.visualization.AreaChart(document.getElementById('areachartECD'));
        ...
        function resize() {  
          ...
          chart_valEC5.clearChart();
          chart_valEC5.draw(data_valEC5, options_valEC5);
        }
        ...
        buttonEC7D.onclick = function () {
          ...
          data_valEC = google.visualization.arrayToDataTable([
            ['Date', 'EC', 'TDS'],
            <?php // PHP Google Charts
            $result31 = $con->query($queryE7D);
            if ($result31->num_rows > 0) {
              // output data of each row
              while($row31 = $result31->fetch_assoc()) {
                echo "['".$row31['date']."',".$row31['E'].",".$row31['T']."],";
              }
            }?>]);
            data_valEC5 = google.visualization.arrayToDataTable([
            ['Date', 'EC'],
            <?php         
            $result51 = $con->query($queryM7D);
            if ($result51->num_rows > 0) {
            // output data of each row
              while($row51 = $result51->fetch_assoc()) {
                echo "['".$row51['data']."',".$row51['media']."],";
              }
            }?>]);
            chart_valEC.clearChart(); 
            chart_valEC.draw(data_valEC, options_valEC);
