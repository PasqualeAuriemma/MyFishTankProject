  ...
  <body class="homepage is-preload">
    ...
    <script type="text/javascript">
      ...
      function setDataForAreaChart(){
        ...
        $year = date('Y');
        $query4 = "SELECT AVG(ph) as media FROM my_myfishtank.ph_tab WHERE FROM_UNIXTIME(data_send, '%Y-%m-%d') = '$todayDate'";
        $query2 = "SELECT ROUND(ph, 1) AS RoundPHValue, count(id) as count FROM my_myfishtank.ph_tab GROUP BY RoundPHValue";
      ...
      function drawPieAreaColumnCharts() {
        ...
        $formatted_temperature = number_format($media_temp, 2);
        echo "
        var options_val = {  
          height: '1400px',
          width: '100px',
          title: 'PH daily average is $formatted_temperature and PH trend is:',
        ...
        data_val2 = google.visualization.arrayToDataTable([
          ...
          if ($result2->num_rows > 0) {
            ...
            while($row2 = $result2->fetch_assoc()) {
              echo "['".$row2['RoundPHValue']."',".$row2['count']."],";
            }
          }
          ?>
        ]);
